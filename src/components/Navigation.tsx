...
const NAV_KEYS = ["nav.home", "nav.language", "nav.library", "nav.novels", "nav.vocabulary", "nav.settings"];

export function Navigation() {
  const pathname = usePathname();
  const { locale, t, LANGUAGES_LIST } = useLocale();
  const currentLang = LANGUAGES_LIST.find((l) => l.code === locale);
    ...
    <nav className="fixed left-0 top-0 h-full w-72 glass flex flex-col z-50">
        ...
        {NAV_ITEMS.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <span className="font-medium">{t[navKey] || item.href}</span>
              </div>
            </Link>
          );
        })}
      ...
      <div className="p-4 mx-3 mb-3 rounded-xl bg-white/[0.02] border border-white/5">
        <p className="text-xs text-muted flex items-center gap-2">
          ...
          AI-Powered {t["nav.language"]} Learning
        </p>
